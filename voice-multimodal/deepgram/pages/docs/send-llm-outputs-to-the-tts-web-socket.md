...
source: https://developers.deepgram.com/docs/send-llm-outputs-to-the-tts-web-socket.md
path: docs/send-llm-outputs-to-the-tts-web-socket
---
...
public class SimpleTextToSpeechWs {
    public static void main(String[] args) throws Exception {
        ...
        wsClient.onSpeakV1Audio(audioData -> {
            ...
            System.out.println("Received " + bytes.length + " bytes of audio");
        });

        wsClient.onFlushed(flushed ->
            System.out.println("Flushed received")
        );
        ...
        // Send each text segment
        for (String text : story) {
            System.out.println("Sending: " + text);
            wsClient.sendText(SpeakV1Text.builder().text(text).build());
        }

        // Flush to receive remaining audio
        System.out.println("Flushing...");
        wsClient.sendFlush(
            SpeakV1Flush.builder().type(SpeakV1FlushType.FLUSH).build()
        );

        // Wait for audio to arrive, then close
        Thread.sleep(5000);
        wsClient.close();
    }
}
...
public class ChatGptToTtsWs {
    public static void main(String[] args) throws Exception {
        // OpenAI client (reads OPENAI_API_KEY from env)
        OpenAIClient openai = OpenAIClient.builder().build();

        // Deepgram TTS WebSocket client (reads DEEPGRAM_API_KEY from env)
        DeepgramClient deepgram = DeepgramClient.builder().build();
        V1WebSocketClient wsClient = deepgram.speak().v1().v1WebSocket();
        ...
        String question = "Tell me a story about a dragon.";

        // Stream ChatGPT response and forward each chunk to Deepgram TTS
        openai.chat().completions().createStreaming(
            ChatCompletionCreateParams.builder()
                .model("gpt-4o-mini")
                .addMessage(ChatCompletionSystemMessageParam.builder()
                    .content("You are ChatGPT, an AI assistant. Make your responses concise.")
                    .build())
         ...
         .forEach(choice -> {
             ...
             if (!content.isEmpty()) {
                 ...
                 wsClient.sendText(
                     SpeakV1Text.builder().text(content).build()
                 );
             }
         });
...
DEFAULT_URL = (
    f"wss://api.deepgram.com/v1/speak?model=aura-2-thalia-en&encoding=linear16&sample_rate={RATE}"
)
DEFAULT_DEEPGRAM_TOKEN = os.environ.get("DEEPGRAM_API_KEY", None)
...
public class AnthropicToTtsWs {
    public static void main(String[] args) throws Exception {
        // Anthropic client (reads ANTHROPIC_API_KEY from env)
        AnthropicClient anthropic = AnthropicClient.builder().build();
        ...
        // Stream Claude response and forward each chunk to Deepgram TTS
        anthropic.messages().createStreaming(
         ...
         .forEach(event -> {
             ...
             if (text != null && !text.isEmpty()) {
                 ...
                 wsClient.sendText(
                     SpeakV1Text.builder().text(text).build()
                 );
             }
         });
