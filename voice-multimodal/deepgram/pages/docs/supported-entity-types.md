...
Deepgram's [entity detection](/docs/detect-entities) model recognizes 59 entity types. [Redaction](/docs/redaction) supports 56 of them. The tables below list every entity type that can be redacted, divided into four groups: PII (Personally Identifiable Information), PHI (Protected Health Information), PCI (Payment Card Industry), and Other Entities.

> **Info**
>
> Three entity types are returned by entity detection but are not valid `redact` values: `cardinal`, `ordinal`, and `percent`. Passing any of them to `redact` returns a 400 error.
...
The `pii` group includes every entity type marked `pii` in any table below, including the Other Entities table, not only the PII table. Two narrower selectors are also accepted: `expiration` redacts only `credit_card_expiration`, and `pin` redacts only `password`.
