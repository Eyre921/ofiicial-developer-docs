...
source: https://developers.deepgram.com/docs/aws-k8s.md
path: docs/aws-k8s
---
...
1. Download a `ClusterConfig` template from Deepgram's [self-hosted resources](https://github.com/deepgram/self-hosted-resources/tree/main/charts/deepgram-self-hosted/samples/). For example, [here](https://github.com/deepgram/self-hosted-resources/blob/main/charts/deepgram-self-hosted/samples/01-basic-setup-aws.cluster-config.yaml) is a template for a basic setup on AWS.
   ...
   1. Set the cluster name (`metadata.name`) and region according to your needs.
   2. Ensure that the IAM role name for Amazon EFS is unique in your AWS account `iam.serviceAccounts[1].roleName`.
   3. Modify each managed node group's `desiredCapacity` according to your needs. You may wish to consult your Deepgram Account Representative in planning your cluster's capacity.
   ...
   ```shell Shell
    FS_ID=$(
      aws efs create-file-system \
          --encrypted \
          --creation-token "$CLUSTER_NAME-resources" \
          --tags Key=Name,Value="$CLUSTER_NAME-resources" \
                  Key=associated-cluster-name,Value=$CLUSTER_NAME \
          --query 'FileSystemId' \
          --output text \
      || \
      aws efs describe-file-systems \
          --query "FileSystems[?Tags[?Key == 'associated-cluster-name' && Value == '$CLUSTER_NAME']].FileSystemId | [0]" \
          --output text
    )
   ```
