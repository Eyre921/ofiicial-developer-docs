...
* The agent requires a [dynamic variable](/docs/eleven-agents/customization/personalization/dynamic-variables) that has no value. Inbound WhatsApp conversations start with no user-provided dynamic variables — only the system variables are populated. The fix is a [conversation initiation webhook](/docs/eleven-agents/customization/personalization/twilio-personalization) that returns every variable the agent requires; see [initialization context](/docs/eleven-agents/whatsapp#initialization-context). The values entered under **Dynamic Variables** in the agent editor are test placeholders and are not used in production.
