---
title: "Update test"
source: https://elevenlabs.io/docs/eleven-agents/api-reference/tests/update.md
path: docs/eleven-agents/api-reference/tests/update
---
...
Reference: https://elevenlabs.io/docs/eleven-agents/api-reference/tests/update
...
This endpoint expects a TestsUpdateRequestBody.

- `TestsUpdateRequestBody`
  - `type`: `llm`
    - `name` (string, required)
    - `chat_history` (list of ConversationHistoryTranscriptCommonModelInput, optional)
    ...
    - `from_conversation_metadata` (TestFromConversationMetadataInput, optional) — Metadata of a conversation this test was created from (if applicable).
    - `parent_folder_id` (string, optional) — The ID of the parent folder. If not provided, the test will be moved to the root level.
  ...
  - `type`: `tool`
    ...
    - `from_conversation_metadata` (TestFromConversationMetadataInput, optional) — Metadata of a conversation this test was created from (if applicable).
    - `parent_folder_id` (string, optional) — The ID of the parent folder. If not provided, the test will be moved to the root level.
    - `tool_call_parameters` (UnitTestToolCallEvaluationModelInput, optional) — How to evaluate the agent's tool call (if any). If empty, the tool call is not evaluated.
  - `type`: `simulation`
    - `name` (string, required)
    - `chat_history` (list of ConversationHistoryTranscriptCommonModelInput, optional)
    - `conversation_initiation_source` (enum, optional, default: unknown) — Simulate the test as if the conversation originated from this channel.
    ...
    - `tool_mock_overrides` (map from string to list of ToolResponseMockConfigInput, optional) — Test-specific response mocks, keyed by tool ID. Applied ahead of the tool's shared mocks and only within this test. Only take effect for tools that are mocked (see tool_mock_config).
...
### 422 Tests Update Request Unprocessable Entity Error
...
- `original_agent_reply` (list of ConversationHistoryTranscriptCommonModelInput, optional, default: [])
