      ...
      - `type`: `integration`
        - `value` (object, required)
          ...
          - `integration_type`: `slack`
            - `channel_id` (string, required) — ID of the Slack channel to post alert notifications to, e.g. ``C0123456789``. The Slack app must be a member of the channel and have the ``chat:write`` scope, or ``chat:write.public`` for public channels it has not joined.
            - `connection_id` (string, required) — ID of the workspace integration connection to deliver alert lifecycle notifications to. The connection's integration must have the monitoring capability and match ``integration_type``.
            - `type` ("integration", optional)
      - `type`: `webhook`
