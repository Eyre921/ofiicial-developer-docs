...
### 422 Batch Calls Get Request Unprocessable Entity Error
...
- `conversation_initiation_client_data` (ConversationInitiationClientDataInternal, optional)
...
- `tool_mock_overrides` (map from string to list of ToolResponseMockConfigOutput, optional) — Per-tool response mock overrides keyed by resolved tool name, applied ahead of the tool's shared mocks. Used for test-specific mocks.
