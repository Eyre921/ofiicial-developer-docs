...
- `request_headers` (map from string to string or object or object or object, optional) — The headers to include in requests to the MCP server
...
- `secret_token` (object, optional) — Optional secret token for authentication with this MCP server
  - `secret_id` (string, required)
