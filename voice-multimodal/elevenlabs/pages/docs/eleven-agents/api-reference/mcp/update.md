...
source: https://elevenlabs.io/docs/eleven-agents/api-reference/mcp/update.md
path: docs/eleven-agents/api-reference/mcp/update
---
...
- `approval_policy` (enum, optional, default: require_approval_all) — The approval mode to set for the MCP server
...
- `response_timeout_secs` (integer, optional) — The maximum time in seconds to wait for each MCP tool call to complete.
- `request_headers` (map from string to McpServerConfigUpdateRequestModelRequestHeadersValue, optional) — The headers to include in requests to the MCP server
- `request_meta` (map from string to McpServerConfigUpdateRequestModelRequestMetaValue, optional) — Entries sent in the MCP `_meta` field of tools/call requests. Values may be JSON scalars, or references to a workspace secret, dynamic variable, or environment variable resolved per call.
- `disable_compression` (boolean, optional) — Whether to disable HTTP compression for this MCP server
- `secret_token` (ConvAiSecretLocator, optional) — Optional secret token for authentication with this MCP server
- `auth_connection` (McpServerConfigUpdateRequestModelAuthConnection, optional) — Optional auth connection to use for authentication with this MCP server
...
### 422 Mcp Servers Update Request Unprocessable Entity Error
