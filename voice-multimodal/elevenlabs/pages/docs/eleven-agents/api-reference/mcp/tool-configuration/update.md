...
- `tool_call_sound` (McpToolConfigOverrideUpdateRequestModelToolCallSound, optional) — Overrides the server's tool_call_sound setting for this tool. A sound name plays that sound; 'off' overrides to no sound (silence); null means do not override (inherit the server default).
...
- `input_overrides` (map from string to McpToolConfigOverrideUpdateRequestModelInputOverridesValue, optional) — Mapping of json path to input override configuration
