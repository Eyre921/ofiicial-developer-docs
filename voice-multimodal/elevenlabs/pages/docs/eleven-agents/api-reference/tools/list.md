---
title: "List tools"
source: https://elevenlabs.io/docs/eleven-agents/api-reference/tools/list.md
path: docs/eleven-agents/api-reference/tools/list
---
...
- `search` (string, optional) — If specified, the endpoint returns only tools whose names start with this string.
...
- `created_by_user_id` (string, optional) — Filter tools by creator user ID. When set, only tools created by this user are returned. Takes precedence over show_only_owned_documents. Use '@me' to refer to the authenticated user.
- `types` (enum, optional) — If present, the endpoint will return only tools of the given types.
