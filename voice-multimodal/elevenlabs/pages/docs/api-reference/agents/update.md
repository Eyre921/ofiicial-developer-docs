...
- `platform_settings` (object, optional) — Platform settings for the agent are all settings that aren't related to the conversation orchestration and content.
  ...
  - `alerting` (object, optional, nullable) — Agent-level alerting configuration overriding workspace settings.
    ...
    - `notifiers` (list of object, optional) — Delivery channels for alert lifecycle notifications. Stacked with other layers and deduped by ``webhook_id``, PagerDuty ``connection_id``, or Slack ``(connection_id, channel_id)``.
