...
source: https://elevenlabs.io/docs/api-reference/studio/convert-chapter.md
path: docs/api-reference/studio/convert-chapter
---
...
- `status` (string, required) — The status of the studio chapter conversion request. If the request was successful, the status will be 'ok'. Otherwise an error message with status 500 will be returned.
...
async function main() {
    ...
    await client.studio.projects.chapters.convert("chapter_id", "project_id");
}
main();
