...
source: https://elevenlabs.io/docs/api-reference/voices/pvc/samples/delete.md
path: docs/api-reference/voices/pvc/samples/delete
---
...
- `status` (string, required) — The status of the voice sample deletion request. If the request was successful, the status will be 'ok'. Otherwise an error message with status 500 will be returned.
...
async function main() {
    ...
    await client.voices.pvc.samples.delete("sample_id", "voice_id");
}
main();
