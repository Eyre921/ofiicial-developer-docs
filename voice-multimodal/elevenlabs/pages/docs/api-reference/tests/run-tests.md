...
source: https://elevenlabs.io/docs/api-reference/tests/run-tests.md
path: docs/api-reference/tests/run-tests
---
...
This endpoint expects a RunAgentTestsRequestModel.

- `tests` (list of SingleTestRunRequestModel, required) — List of tests to run on the agent
- `agent_config_override` (AdhocAgentConfigOverrideForTestRequestModel, optional, nullable) — Configuration overrides to use for testing. If not provided, the agent's default configuration will be used.
- `branch_id` (string, optional, nullable) — ID of the branch to run the tests on. If not provided, the tests will be run on the agent's main branch.
...
- `workflow_node_id` (string, optional, nullable) — ID of the workflow node to run the test on. If not provided, the test will be run on the agent's default workflow node.
- `root_folder_id` (string, optional, nullable) — ID of the root folder to run the test on. If not provided, the test will be run on the agent's default folder.
- `root_folder_name` (string, optional, nullable) — Name of the root folder to run the test on. If not provided, the test will be run on the agent's default folder.

### AdhocAgentConfigOverrideForTestRequestModel

- `conversation_config` (ConversationalConfigAPIModel-Input, required)
- `platform_settings` (AgentPlatformSettingsRequestModel, required)
- `workflow` (AgentWorkflowRequestModel, optional, nullable)
