...
source: https://elevenlabs.io/docs/api-reference/flows/templates/list.md
path: docs/api-reference/flows/templates/list
---
...
GET https://api.elevenlabs.io/v1/flows/templates
...
Reference: https://elevenlabs.io/docs/api-reference/flows/templates/list
...
- `page_size` (integer, optional, default: 10) — How many templates to return per page. Lower than the run list's ceiling because each row expands its versions' input and output schemas.
- `versions_per_template` (integer, optional, default: 3) — How many of each template's published versions to return, newest first. `has_more_versions` tells you when a template has more.
- `search` (string, optional, nullable) — Only return templates whose name or description contains this text.
...
- `templates` (list of TemplateSummary, required) — The templates on this page, most recently updated first. A page can hold fewer than `page_size` templates, or none at all, because templates you cannot run are filtered out after the page is read — keep paging while `has_more` is true.
...
url = URI("https://api.elevenlabs.io/v1/flows/templates")
...
HttpResponse<String> response = Unirest.get("https://api.elevenlabs.io/v1/flows/templates")
  .asString();
```
...
$response = $client->request('GET', 'https://api.elevenlabs.io/v1/flows/templates');
...
var client = new RestClient("https://api.elevenlabs.io/v1/flows/templates");
...
let request = NSMutableURLRequest(url: NSURL(string: "https://api.elevenlabs.io/v1/flows/templates")! as URL,
