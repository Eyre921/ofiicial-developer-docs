...
source: https://elevenlabs.io/docs/api-reference/mcp/update.md
path: docs/api-reference/mcp/update
---
...
This endpoint expects a MCPServerConfigUpdateRequestModel.

- `approval_policy` (enum, optional, nullable, default: require_approval_all) — The approval mode to set for the MCP server
...
- `tool_call_sound_behavior` (enum, optional, nullable, default: auto) — Determines when the tool call sound should play for all tools from this MCP server
...
- `response_timeout_secs` (integer, optional, nullable) — The maximum time in seconds to wait for each MCP tool call to complete.
- `request_headers` (map from string to McpServerConfigUpdateRequestModelRequestHeaders, optional, nullable) — The headers to include in requests to the MCP server
- `request_meta` (map from string to McpServerConfigUpdateRequestModelRequestMeta, optional, nullable) — Entries sent in the MCP `_meta` field of tools/call requests. Values may be JSON scalars, or references to a workspace secret, dynamic variable, or environment variable resolved per call.
- `disable_compression` (boolean, optional, nullable) — Whether to disable HTTP compression for this MCP server
- `secret_token` (ConvAISecretLocator, optional, nullable) — Optional secret token for authentication with this MCP server
- `auth_connection` (McpServerConfigUpdateRequestModelAuthConnection, optional, nullable) — Optional auth connection to use for authentication with this MCP server
