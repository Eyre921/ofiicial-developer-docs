...
source: https://upstash.com/docs/workflow/howto/retry-now
path: docs/workflow/howto/retry-now
---
...
branch has its own button.
  ...
  <img alt="Run now and Retry now buttons on workflow steps in the Upstash Console" />
...
<Note>
  ...
  [failure function](/docs/workflow/howto/failures) is called as usual.
</Note>

## From the API

Under the hood, a sleeping or retrying step is a delayed QStash message. The
console calls the [retry a step now](/docs/workflow/api-reference/runs/retry-a-step-now)
endpoint with the step's message ID, which you can find in the run's
[event logs](/docs/workflow/api-reference/logs/list-workflow-run-logs).
