...
<Note>
  The browser can only be provisioned when the box is created. It cannot be enabled on an existing box. See [Browser](/docs/box/overall/browser/overview) for the full surface.
</Note>
...
<Note>
  `extract` uses an LLM and is metered. It needs a model provider key on the box or your account. See [AI Actions](/docs/box/overall/browser/ai-actions) for the model override and the full list of providers.
</Note>
...
* [Reading pages](/docs/box/overall/browser/reading-pages) for `content`, `screenshot`, and `extract`.
* [AI actions](/docs/box/overall/browser/ai-actions) for `observe` and `act`, including replaying a resolved action with no LLM.
* [Live view](/docs/box/overall/browser/live-view) and [recordings](/docs/box/overall/browser/recordings) to watch or replay a scrape.
* [Browser cookbook](/docs/box/overall/browser/cookbook) for runnable examples across crawling, automation, and testing.
