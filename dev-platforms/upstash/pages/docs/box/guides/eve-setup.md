...
The sandbox runs model-generated code, so egress is [deny-all](/docs/box/overall/network-policy) by default. Open it where you need it, in the `use(...)` call, never as a backend-level setting.
...
A `bootstrap` hook runs once, and Eve caches the result as a template so later sessions start from it. On Box that template is a [snapshot](/docs/box/overall/snapshots).
...
Boxes use Box's pause-based idle lifecycle by default (`keepAlive: false`): auto-paused when idle, resumed on reattach, and reaped by Box. Pass [`keepAlive: true`](/docs/box/overall/keep-alive) only when you want an always-running box that you manage and delete yourself.
...
* [Snapshots](/docs/box/overall/snapshots) for building and restoring the boxes behind templates.
