---
title: "Reading"
source: https://upstash.com/docs/blob/bucket/reading
path: docs/blob/bucket/reading
---
...
The records these return (`BlobObject`, `BlobInfo`, `BlobDownload`) are on [Types](/docs/blob/reference/types#records).
...
A missing object throws a `BlobError` with code `not_found`, status 404. See [Errors](/docs/blob/reference/errors).

There is no range option. For byte ranges, use an [S3 client](/docs/blob/bucket/connecting#using-an-s3-client).
...
`metadata` comes back from `get` and `info` only, with keys lowercased. See [Metadata](/docs/blob/bucket/writing#metadata).
...
Pair it with `cache: 'immutable'` at upload. See [Caching](/docs/blob/bucket/caching#immutable-plus-a-versioned-url).
