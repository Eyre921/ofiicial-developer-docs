---
title: "Writing"
source: https://upstash.com/docs/blob/bucket/writing
path: docs/blob/bucket/writing
---
...
`bucket` is the client from [Connecting](/docs/blob/bucket/connecting). Everything here runs on your server with the bucket token.

For files a user picks in the browser, do not proxy the bytes through your app. Use an [upload handler](/docs/blob/uploads/upload-handler) instead.
...
`Size` is a decimal byte count like `4096` or `'20mb'` ([Types](/docs/blob/reference/types#size)). The content type grammar and wildcards are on [Constraints](/docs/blob/uploads/constraints).
...
This is a `CompletedBlob` ([Types](/docs/blob/reference/types#records)). On a private bucket `url` and `versionedUrl` are `undefined`.
...
When proxying bytes through a route, keep `maxSize` under the platform's own request body cap, since that refusal happens before your route runs. See [Platform body limits](/docs/blob/reference/errors#platform-body-limits).
...
A move is not atomic. If the copy lands and the delete fails, `move` throws `move_left_a_copy` and keeps both objects. Retry the source delete to recover; see [Deleting](/docs/blob/bucket/deleting#move-leaves-a-copy-on-failure).
...
A body over the [multipart threshold](/docs/blob/uploads/large-files#the-multipart-threshold) of 16 MB goes up in parts instead of one PUT. `multipart` changes the threshold: a size sets a new one, `true` always uses parts, `false` never does.
...
`expiresAt` may be sooner than what you asked for. Cache the link until then rather than computing your own deadline; see [Use expiresAt](/docs/blob/bucket/reading#use-expiresat-not-expiresin).

For a browser upload, use the [upload handler](/docs/blob/uploads/upload-handler) instead. A signed URL is one PUT: no multipart, no resume, and nothing tells your server it happened.
