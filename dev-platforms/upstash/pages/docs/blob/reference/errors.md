---
title: "Errors"
source: https://upstash.com/docs/blob/reference/errors
path: docs/blob/reference/errors
---
...
A route runs [`onError`](/docs/blob/uploads/upload-handler#onerror) first. If it returns a `Response`, that is the answer; if it returns a `BlobError`, the answer is that error's JSON at its own status. Otherwise the throw falls through three cases:
...
A throw out of `onUploadComplete` also deletes the completed object before the error is answered. See [onUploadComplete](/docs/blob/uploads/upload-handler#onuploadcomplete).
...
**A 403 on a freshly minted presign** becomes `signature_mismatch`. A 401 or 403 on an older URL is read as an expired signature and the browser asks the route for a new one instead. See [Retries](/docs/blob/uploads/large-files#retries).
...
Credentials are short-lived and re-minted before they expire. One that expires mid-request is handled inside the SDK, so only a second refusal surfaces. See [How signing works](/docs/blob/reference/signing).
