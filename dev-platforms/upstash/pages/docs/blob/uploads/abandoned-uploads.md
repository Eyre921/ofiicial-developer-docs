...
source: https://upstash.com/docs/blob/uploads/abandoned-uploads
path: docs/blob/uploads/abandoned-uploads
---
...
What is left depends on which side of the [multipart threshold](/docs/blob/uploads/large-files#the-multipart-threshold) the file was on:
...
On an all-multipart app this route is the whole cleanup. The calls it is built from are on [Deleting](/docs/blob/bucket/deleting#incomplete-multipart-uploads).
...
* **Do not let `onUploadComplete` throw on a database error.** Any throw deletes the object, and the browser's retried `end` then reports a 404. See [Retries and throws](/docs/blob/uploads/upload-handler#retries-and-throws).
* **Use unique paths unless overwriting is the intent.** Two single-PUT uploads to the same path race, and the loser's `end` fails with `not_found` even though its bytes landed. `uniquePath` is the fix; see [Writing](/docs/blob/bucket/writing#uniquepath).
