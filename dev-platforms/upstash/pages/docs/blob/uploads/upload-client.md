...
| `constraints` | What the route's [`GET`](/docs/blob/uploads/upload-handler#the-get-endpoint) served. `undefined` until it answers. |
...
Drive UI off `pending` rather than deriving it from `status`. `percent` stays at 99 through `finishing`, because 100 means stored, not sent. See [Progress and status](/docs/blob/uploads/large-files#progress-and-status).

`blob.data` is typed from that route's [`onUploadComplete`](/docs/blob/uploads/upload-handler#onuploadcomplete). Fields a status does not carry are `undefined` rather than absent, so `upload?.blob?.url` and `upload?.error?.message` read straight off the record with no narrowing.

`canPause` is `false` for a single PUT, which is every file under the multipart threshold. `retry()` works only from `error`, and resumes from the parts that already landed. See [Large files](/docs/blob/uploads/large-files).
...
A proxied upload is capped by your platform's request body limit rather than by `maxSize`. The SDK surfaces that refusal as `too_large` with the platform's limit as a hint; see [Platform body limits](/docs/blob/reference/errors#platform-body-limits). Anything larger needs a direct upload with an [upload handler](/docs/blob/uploads/upload-handler).
