---
title: "Large Files"
source: https://upstash.com/docs/blob/uploads/large-files
path: docs/blob/uploads/large-files
---
...
The default is **16 MB decimal**, 16,000,000 bytes. A 16,000,000 byte body is a single PUT; 16,000,001 is multipart. [Sizes](/docs/blob/reference/types#size) are decimal everywhere. `bucket.put()` on your server and a direct browser upload use the same threshold.
...
The row that matters most is when the object comes into existence. Under the threshold the object is stored the moment the last byte lands, before your route has been told anything. Over the threshold nothing exists at the path until the upload is completed. See [Abandoned uploads](/docs/blob/uploads/abandoned-uploads) for what that means when a tab closes.
...
* On `bucket.put()`, `allowOverwrite: false` and `ifUnchanged` are single-PUT only; see [Conditional writes](/docs/blob/bucket/writing#conditional-writes).
