---
title: "XGROUP"
source: https://upstash.com/docs/redis/commands/streams/xgroup
path: docs/redis/commands/streams/xgroup
---
...
`CREATECONSUMER` and `DELCONSUMER` add and remove consumers explicitly; consumers are otherwise created on first read. Deleting a consumer returns how many pending entries went away with it, and those entries are then no longer pending for anyone, so hand them over with [`XCLAIM`](/docs/redis/commands/streams/xclaim) or [`XAUTOCLAIM`](/docs/redis/commands/streams/xautoclaim) first if the work still matters.
