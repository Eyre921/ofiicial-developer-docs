---
title: "AI Tools"
source: https://upstash.com/docs/agent-resources/overview
path: docs/agent-resources/overview
---
...
Don't see your agent? See the [full list of agents](/docs/agent-resources/clients).
...
Prefer a headless setup, or need [Upstash Box](/docs/box/overall/quickstart) tools? Run the local server with a [Developer API key](/docs/devops/developer-api/introduction#create-an-api-key):

```bash
npx -y @upstash/mcp-server@latest --email YOUR_EMAIL --api-key YOUR_API_KEY
```

See the [MCP server page](/docs/agent-resources/mcp) for remote vs. local, authentication, and feature options.

<Note>
  Only working against a single Redis database? The lightweight [Redis MCP](/docs/redis/sdks/mcp) needs just that database's REST URL and token, not an account API key.
</Note>
