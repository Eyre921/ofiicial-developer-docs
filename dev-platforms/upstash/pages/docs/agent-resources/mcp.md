---
title: "MCP Server"
source: https://upstash.com/docs/agent-resources/mcp
...
For the MCP to interact with [Upstash Box](/docs/box/overall/quickstart), the agent needs your Box API key. By default you have to paste it into the chat (or keep it in a `.env`) every time the agent runs a Box tool. To avoid this, wire the key into the MCP setup itself so the server picks it up on startup, either as a CLI flag or an environment variable.
