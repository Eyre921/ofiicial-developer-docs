# dev-platforms/sentry 文档索引

> 共 118 篇，目录严格镜像官方 URL 路径。完整合并版见 [`llms-full.txt`](./llms-full.txt)。


## (根目录)

- `changelog` — [Documentation Changelog](pages/changelog.md) · [原文](https://docs.sentry.io/changelog.md)
- `contributing` — [Contributing to Docs](pages/contributing.md) · [原文](https://docs.sentry.io/contributing.md)

## account

- `account/auth-tokens` — [Auth Tokens](pages/account/auth-tokens.md) · [原文](https://docs.sentry.io/account/auth-tokens.md)
- `account/user-settings` — [Account Preferences](pages/account/user-settings.md) · [原文](https://docs.sentry.io/account/user-settings.md)

## ai

- `ai/agent-plugin` — [Agent Plugin](pages/ai/agent-plugin.md) · [原文](https://docs.sentry.io/ai/agent-plugin.md)
- `ai/dotagents` — [dotagents](pages/ai/dotagents.md) · [原文](https://docs.sentry.io/ai/dotagents.md)

## api

- `api/auth` — [Authentication](pages/api/auth.md) · [原文](https://docs.sentry.io/api/auth.md)
- `api/crons` — [Crons](pages/api/crons.md) · [原文](https://docs.sentry.io/api/crons.md)
- `api/dashboards` — [Dashboards](pages/api/dashboards.md) · [原文](https://docs.sentry.io/api/dashboards.md)
- `api/discover` — [Discover & Performance](pages/api/discover.md) · [原文](https://docs.sentry.io/api/discover.md)
- `api/environments` — [Environments](pages/api/environments.md) · [原文](https://docs.sentry.io/api/environments.md)
- `api/events` — [Events & Issues](pages/api/events.md) · [原文](https://docs.sentry.io/api/events.md)
- `api/explore` — [Explore](pages/api/explore.md) · [原文](https://docs.sentry.io/api/explore.md)
- `api/integration` — [Integration Platform](pages/api/integration.md) · [原文](https://docs.sentry.io/api/integration.md)
- `api/integrations` — [Integrations](pages/api/integrations.md) · [原文](https://docs.sentry.io/api/integrations.md)
- `api/mobile-builds` — [Mobile Builds](pages/api/mobile-builds.md) · [原文](https://docs.sentry.io/api/mobile-builds.md)
- `api/monitors` — [Monitors & Alerts](pages/api/monitors.md) · [原文](https://docs.sentry.io/api/monitors.md)
- `api/organizations` — [Organizations](pages/api/organizations.md) · [原文](https://docs.sentry.io/api/organizations.md)
- `api/pagination` — [Paginating Results](pages/api/pagination.md) · [原文](https://docs.sentry.io/api/pagination.md)
- `api/permissions` — [Permissions & Scopes](pages/api/permissions.md) · [原文](https://docs.sentry.io/api/permissions.md)
- `api/profiling` — [Profiling](pages/api/profiling.md) · [原文](https://docs.sentry.io/api/profiling.md)
- `api/projects` — [Projects](pages/api/projects.md) · [原文](https://docs.sentry.io/api/projects.md)
- `api/ratelimits` — [Rate Limits](pages/api/ratelimits.md) · [原文](https://docs.sentry.io/api/ratelimits.md)
- `api/releases` — [Releases](pages/api/releases.md) · [原文](https://docs.sentry.io/api/releases.md)
- `api/replays` — [Replays](pages/api/replays.md) · [原文](https://docs.sentry.io/api/replays.md)
- `api/requests` — [Requests](pages/api/requests.md) · [原文](https://docs.sentry.io/api/requests.md)
- `api/scim` — [SCIM](pages/api/scim.md) · [原文](https://docs.sentry.io/api/scim.md)
- `api/seer` — [Seer](pages/api/seer.md) · [原文](https://docs.sentry.io/api/seer.md)
- `api/snapshots` — [Snapshots](pages/api/snapshots.md) · [原文](https://docs.sentry.io/api/snapshots.md)
- `api/spike-protection` — [Spike Protection Notifications](pages/api/spike-protection.md) · [原文](https://docs.sentry.io/api/spike-protection.md)
- `api/teams` — [Teams](pages/api/teams.md) · [原文](https://docs.sentry.io/api/teams.md)
- `api/users` — [Users](pages/api/users.md) · [原文](https://docs.sentry.io/api/users.md)

## cli

- `cli/code-mappings` — [Code Mappings](pages/cli/code-mappings.md) · [原文](https://docs.sentry.io/cli/code-mappings.md)
- `cli/configuration` — [Configuration and Authentication](pages/cli/configuration.md) · [原文](https://docs.sentry.io/cli/configuration.md)
- `cli/crons` — [Crons (CLI)](pages/cli/crons.md) · [原文](https://docs.sentry.io/cli/crons.md)
- `cli/dif` — [Debug Information Files](pages/cli/dif.md) · [原文](https://docs.sentry.io/cli/dif.md)
- `cli/installation` — [Installation](pages/cli/installation.md) · [原文](https://docs.sentry.io/cli/installation.md)
- `cli/logs` — [Logs](pages/cli/logs.md) · [原文](https://docs.sentry.io/cli/logs.md)
- `cli/releases` — [Release Management](pages/cli/releases.md) · [原文](https://docs.sentry.io/cli/releases.md)
- `cli/send-event` — [Sending Events](pages/cli/send-event.md) · [原文](https://docs.sentry.io/cli/send-event.md)
- `cli/snapshots` — [Snapshots (CLI)](pages/cli/snapshots.md) · [原文](https://docs.sentry.io/cli/snapshots.md)

## concepts

- `concepts/data-management` — [Data Management](pages/concepts/data-management.md) · [原文](https://docs.sentry.io/concepts/data-management.md)
- `concepts/key-terms` — [Concepts](pages/concepts/key-terms.md) · [原文](https://docs.sentry.io/concepts/key-terms.md)
- `concepts/migration` — [Moving to SaaS](pages/concepts/migration.md) · [原文](https://docs.sentry.io/concepts/migration.md)
- `concepts/otlp` — [OpenTelemetry Protocol (OTLP)](pages/concepts/otlp.md) · [原文](https://docs.sentry.io/concepts/otlp.md)
- `concepts/search` — [Search](pages/concepts/search.md) · [原文](https://docs.sentry.io/concepts/search.md)

## get-started

- `get-started/cookbook` — [Sentry Cookbook](pages/get-started/cookbook.md) · [原文](https://docs.sentry.io/get-started/cookbook.md)
- `get-started/guides` — [Guides](pages/get-started/guides.md) · [原文](https://docs.sentry.io/get-started/guides.md)

## integrations

- `integrations/cloud-monitoring` — [Cloud Monitoring](pages/integrations/cloud-monitoring.md) · [原文](https://docs.sentry.io/integrations/cloud-monitoring.md)
- `integrations/coding-agents` — [Coding Agents](pages/integrations/coding-agents.md) · [原文](https://docs.sentry.io/integrations/coding-agents.md)
- `integrations/compliance` — [Compliance](pages/integrations/compliance.md) · [原文](https://docs.sentry.io/integrations/compliance.md)
- `integrations/data-forwarding` — [Data Forwarding](pages/integrations/data-forwarding.md) · [原文](https://docs.sentry.io/integrations/data-forwarding.md)
- `integrations/debugging` — [Debugging](pages/integrations/debugging.md) · [原文](https://docs.sentry.io/integrations/debugging.md)
- `integrations/deployment` — [Deployment](pages/integrations/deployment.md) · [原文](https://docs.sentry.io/integrations/deployment.md)
- `integrations/feature-flag` — [Feature Flags](pages/integrations/feature-flag.md) · [原文](https://docs.sentry.io/integrations/feature-flag.md)
- `integrations/integration-platform` — [Integration Platform](pages/integrations/integration-platform.md) · [原文](https://docs.sentry.io/integrations/integration-platform.md)
- `integrations/issue-tracking` — [Issue Tracking](pages/integrations/issue-tracking.md) · [原文](https://docs.sentry.io/integrations/issue-tracking.md)
- `integrations/notification-incidents` — [Notification & Incidents](pages/integrations/notification-incidents.md) · [原文](https://docs.sentry.io/integrations/notification-incidents.md)
- `integrations/session-replay` — [Third-party Session Replay Integrations](pages/integrations/session-replay.md) · [原文](https://docs.sentry.io/integrations/session-replay.md)
- `integrations/source-code-mgmt` — [Source Code Management](pages/integrations/source-code-mgmt.md) · [原文](https://docs.sentry.io/integrations/source-code-mgmt.md)
- `integrations/third-party-integrations` — [Third-Party Integrations](pages/integrations/third-party-integrations.md) · [原文](https://docs.sentry.io/integrations/third-party-integrations.md)
- `integrations/troubleshooting` — [Troubleshooting Integrations](pages/integrations/troubleshooting.md) · [原文](https://docs.sentry.io/integrations/troubleshooting.md)

## organization

- `organization/authentication` — [Authentication](pages/organization/authentication.md) · [原文](https://docs.sentry.io/organization/authentication.md)
- `organization/data-storage-location` — [Data Storage Location (US or EU)](pages/organization/data-storage-location.md) · [原文](https://docs.sentry.io/organization/data-storage-location.md)
- `organization/dynamic-sampling` — [Dynamic Sampling](pages/organization/dynamic-sampling.md) · [原文](https://docs.sentry.io/organization/dynamic-sampling.md)
- `organization/early-adopter-features` — [Early Adopter Features](pages/organization/early-adopter-features.md) · [原文](https://docs.sentry.io/organization/early-adopter-features.md)
- `organization/getting-started` — [Set Up Your Organization](pages/organization/getting-started.md) · [原文](https://docs.sentry.io/organization/getting-started.md)
- `organization/membership` — [Organization and User Management](pages/organization/membership.md) · [原文](https://docs.sentry.io/organization/membership.md)

## platforms

- `platforms/android` — [Android](pages/platforms/android.md) · [原文](https://docs.sentry.io/platforms/android.md)
- `platforms/apple` — [Apple](pages/platforms/apple.md) · [原文](https://docs.sentry.io/platforms/apple.md)
- `platforms/dart` — [Dart](pages/platforms/dart.md) · [原文](https://docs.sentry.io/platforms/dart.md)
- `platforms/dotnet` — [.NET](pages/platforms/dotnet.md) · [原文](https://docs.sentry.io/platforms/dotnet.md)
- `platforms/elixir` — [Elixir](pages/platforms/elixir.md) · [原文](https://docs.sentry.io/platforms/elixir.md)
- `platforms/go` — [Go](pages/platforms/go.md) · [原文](https://docs.sentry.io/platforms/go.md)
- `platforms/godot` — [Godot Engine](pages/platforms/godot.md) · [原文](https://docs.sentry.io/platforms/godot.md)
- `platforms/java` — [Java](pages/platforms/java.md) · [原文](https://docs.sentry.io/platforms/java.md)
- `platforms/javascript` — [JavaScript](pages/platforms/javascript.md) · [原文](https://docs.sentry.io/platforms/javascript.md)
- `platforms/kotlin` — [Kotlin](pages/platforms/kotlin.md) · [原文](https://docs.sentry.io/platforms/kotlin.md)
- `platforms/native` — [Native](pages/platforms/native.md) · [原文](https://docs.sentry.io/platforms/native.md)
- `platforms/nintendo-switch` — [Nintendo Switch](pages/platforms/nintendo-switch.md) · [原文](https://docs.sentry.io/platforms/nintendo-switch.md)
- `platforms/php` — [PHP](pages/platforms/php.md) · [原文](https://docs.sentry.io/platforms/php.md)
- `platforms/playstation` — [PlayStation](pages/platforms/playstation.md) · [原文](https://docs.sentry.io/platforms/playstation.md)
- `platforms/powershell` — [PowerShell](pages/platforms/powershell.md) · [原文](https://docs.sentry.io/platforms/powershell.md)
- `platforms/python` — [Python](pages/platforms/python.md) · [原文](https://docs.sentry.io/platforms/python.md)
- `platforms/react-native` — [React Native](pages/platforms/react-native.md) · [原文](https://docs.sentry.io/platforms/react-native.md)
- `platforms/ruby` — [Ruby](pages/platforms/ruby.md) · [原文](https://docs.sentry.io/platforms/ruby.md)
- `platforms/rust` — [Rust](pages/platforms/rust.md) · [原文](https://docs.sentry.io/platforms/rust.md)
- `platforms/unity` — [Unity](pages/platforms/unity.md) · [原文](https://docs.sentry.io/platforms/unity.md)
- `platforms/unreal` — [Unreal Engine](pages/platforms/unreal.md) · [原文](https://docs.sentry.io/platforms/unreal.md)
- `platforms/xbox` — [Xbox](pages/platforms/xbox.md) · [原文](https://docs.sentry.io/platforms/xbox.md)

## pricing

- `pricing/quotas` — [Billing Quota Management](pages/pricing/quotas.md) · [原文](https://docs.sentry.io/pricing/quotas.md)

## product

- `product/agents` — [Agents](pages/product/agents.md) · [原文](https://docs.sentry.io/product/agents.md)
- `product/ai-in-sentry` — [AI in Sentry](pages/product/ai-in-sentry.md) · [原文](https://docs.sentry.io/product/ai-in-sentry.md)
- `product/build-distribution` — [Build Distribution](pages/product/build-distribution.md) · [原文](https://docs.sentry.io/product/build-distribution.md)
- `product/dashboards` — [Dashboards](pages/product/dashboards.md) · [原文](https://docs.sentry.io/product/dashboards.md)
- `product/drains` — [Log and Trace Drains](pages/product/drains.md) · [原文](https://docs.sentry.io/product/drains.md)
- `product/errors` — [Errors](pages/product/errors.md) · [原文](https://docs.sentry.io/product/errors.md)
- `product/issues` — [Issues](pages/product/issues.md) · [原文](https://docs.sentry.io/product/issues.md)
- `product/logs` — [Logs](pages/product/logs.md) · [原文](https://docs.sentry.io/product/logs.md)
- `product/mcp-servers` — [MCP Servers](pages/product/mcp-servers.md) · [原文](https://docs.sentry.io/product/mcp-servers.md)
- `product/metrics` — [Application Metrics](pages/product/metrics.md) · [原文](https://docs.sentry.io/product/metrics.md)
- `product/monitors-and-alerts` — [Monitors and Alerts](pages/product/monitors-and-alerts.md) · [原文](https://docs.sentry.io/product/monitors-and-alerts.md)
- `product/notifications` — [Sentry Notifications](pages/product/notifications.md) · [原文](https://docs.sentry.io/product/notifications.md)
- `product/partnership-platform` — [Partnership Platform](pages/product/partnership-platform.md) · [原文](https://docs.sentry.io/product/partnership-platform.md)
- `product/profiling` — [Profiling](pages/product/profiling.md) · [原文](https://docs.sentry.io/product/profiling.md)
- `product/projects` — [Projects](pages/product/projects.md) · [原文](https://docs.sentry.io/product/projects.md)
- `product/relay` — [Relay](pages/product/relay.md) · [原文](https://docs.sentry.io/product/relay.md)
- `product/releases` — [Releases](pages/product/releases.md) · [原文](https://docs.sentry.io/product/releases.md)
- `product/sentry-basics` — [Getting Started With Sentry](pages/product/sentry-basics.md) · [原文](https://docs.sentry.io/product/sentry-basics.md)
- `product/sentry-toolbar` — [Sentry Toolbar](pages/product/sentry-toolbar.md) · [原文](https://docs.sentry.io/product/sentry-toolbar.md)
- `product/session-replay` — [Session Replay](pages/product/session-replay.md) · [原文](https://docs.sentry.io/product/session-replay.md)
- `product/size-analysis` — [Size Analysis](pages/product/size-analysis.md) · [原文](https://docs.sentry.io/product/size-analysis.md)
- `product/snapshots` — [Snapshots](pages/product/snapshots.md) · [原文](https://docs.sentry.io/product/snapshots.md)
- `product/stats` — [Stats](pages/product/stats.md) · [原文](https://docs.sentry.io/product/stats.md)
- `product/trace-explorer` — [Traces and Spans](pages/product/trace-explorer.md) · [原文](https://docs.sentry.io/product/trace-explorer.md)
- `product/user-feedback` — [User Feedback](pages/product/user-feedback.md) · [原文](https://docs.sentry.io/product/user-feedback.md)

## security-legal-pii

- `security-legal-pii/scrubbing` — [Data Scrubbing](pages/security-legal-pii/scrubbing.md) · [原文](https://docs.sentry.io/security-legal-pii/scrubbing.md)
- `security-legal-pii/security` — [Security & Legal](pages/security-legal-pii/security.md) · [原文](https://docs.sentry.io/security-legal-pii/security.md)
