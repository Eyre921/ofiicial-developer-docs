...
dependencies {
  implementation 'io.sentry:sentry-async-profiler:8.52.0'
}
...
```scala
libraryDependencies += "io.sentry" % "sentry" % "8.52.0"
// ___PRODUCT_OPTION_START___ profiling
libraryDependencies += "io.sentry" % "sentry-async-profiler" % "8.52.0"
// ___PRODUCT_OPTION_END___ profiling
```
...
Download the latest version of the `sentry-opentelemetry-agent-8.52.0.jar` from [MavenCentral](https://search.maven.org/artifact/io.sentry/sentry-opentelemetry-agent):

```bash
curl https://repo1.maven.org/maven2/io/sentry/sentry-opentelemetry-agent/8.52.0/sentry-opentelemetry-agent-8.52.0.jar -o sentry-opentelemetry-agent-8.52.0.jar
```

Then run your application with:

```bash
SENTRY_PROPERTIES_FILE=sentry.properties JAVA_TOOL_OPTIONS="-javaagent:sentry-opentelemetry-agent-8.52.0.jar" java -jar your-application.jar
```
