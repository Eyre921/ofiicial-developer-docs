...
| `status`: `pending`, `requires_action`, `completed`, `failed` | `status`: `queued`, `in_progress`, `requires_action`, `completed`, `failed`, `canceled`, `terminated` |
...
A thread's single `pending` status is now split into `queued` and `in_progress`, and cancellation and termination are reported separately from failure. Treat any status you don't recognize as non-terminal rather than as an error.
