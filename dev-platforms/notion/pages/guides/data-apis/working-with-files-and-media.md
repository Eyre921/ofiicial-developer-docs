...
path: guides/data-apis/working-with-files-and-media
---
...
| **CAD and 3D** | .dwg, .dxf, .dwf, .rvt, .rfa, .fbx, .3dm, .step, .stp, .iges, .igs, .stl, .obj, .3mf, .gltf, .glb, .dae, .usdz                                                                                                                                                                         | image/vnd.dwg, image/vnd.dxf, model/vnd.dwf, application/vnd.autodesk.revit, application/vnd.autodesk.fbx, application/vnd.mcneel.3dm, model/step, model/iges, model/stl, model/obj, model/3mf, model/gltf+json, model/gltf-binary, model/vnd.collada+xml, model/vnd.usdz+zip                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |

CAD and 3D model files attach as `file` blocks, the same as documents.

Some HTTP clients label a file `application/octet-stream` when their MIME database has no entry for its extension. For any upload, the API treats that generic type as unspecified and resolves the real type from the filename extension or the `content_type` recorded when the file upload was created. If neither gives a supported type, the request fails.
