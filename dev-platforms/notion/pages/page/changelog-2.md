...
<Update label="September 2, 2026">
  ...
  [Create a database](/reference/create-database) now accepts an optional `database_type` of `tasks`, `projects`, or `skills`. Notion builds the database from its canonical schema and metadata for that type, so the result matches a database of that type created in the Notion app. `database_type` can't be combined with `initial_data_source`, and when `title` is omitted the database is named after its type. See [Typed databases](/reference/create-database#typed-databases) for the properties each type creates. [Database](/reference/database) and [data source](/reference/data-source) objects now include a read-only `database_type` field.
</Update>
