---
title: "Bulk actions"
source: https://trigger.dev/docs/management/runs/bulk-actions
path: docs/management/runs/bulk-actions
---

Cancel or replay many runs from the SDK using run IDs or the same filters as runs.list().
...
`filter` accepts the same filters as [`runs.list()`](/docs/management/runs/list), excluding pagination fields. Provide at least one filter field; use `runIds` when you want to target specific runs. Relative time filters such as `period` are resolved when the bulk action is created, so later batches process the same fixed time range.
...
<ParamField type="BulkActionFilter">
