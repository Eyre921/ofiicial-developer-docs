...
* **`...chat.toStreamTextOptions()` is spread first.** It wires up the `prepareStep` callback behind [compaction](/docs/ai-chat/compaction), [mid-turn steering](/docs/ai-chat/pending-messages), and [background injection](/docs/ai-chat/background-injection), plus the system prompt set via [`chat.prompt()`](/docs/ai-chat/backend#using-prompts) and telemetry.
