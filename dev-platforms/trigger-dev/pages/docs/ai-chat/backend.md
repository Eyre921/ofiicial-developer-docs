...
Use [AI Prompts](/docs/ai/prompts) to manage your system prompt as versioned, overridable config. Store the resolved prompt in a lifecycle hook with `chat.prompt.set()`. The `streamText` from `run`'s argument picks it up: system prompt, model, config and telemetry.
...
`chat.close()` is available to [custom agents](/docs/ai-chat/custom-agents#ending-the-conversation) too, where the close is performed when your `run()` returns.
