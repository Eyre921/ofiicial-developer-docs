...
source: https://trigger.dev/docs/ai-chat/migrating-from-hydrate-messages
path: docs/ai-chat/migrating-from-hydrate-messages
---
...
<Steps>
  <Step title="Implement load">
    ...
    ```ts lib/transcript-storage.ts theme={"theme":"css-variables"}
  ...
  <Step title="Update the history read on your frontend">
    ...
    <CodeGroup>
      ...
      ```tsx app/c/[chatId]/ChatPage.tsx theme={"theme":"css-variables"}
      "use client";
  ...
  <Step title="Run the conformance suite">
    ...
    ```ts lib/transcript-storage.test.ts theme={"theme":"css-variables"}
...
* **Actions are edits.** `onAction` mutates `chat.history` and returns nothing, or returns `chat.turn()` to have a full turn answer the edited history. The regenerated answer is saved and remembered like any other. See [Actions](/docs/ai-chat/actions#answering-after-an-action).
...
* [Transcript storage](/docs/ai-chat/transcript-storage): the full contract and what `save` receives
* [Actions](/docs/ai-chat/actions): what undo, edit and regenerate become in the changeset
* [Lifecycle hooks](/docs/ai-chat/lifecycle-hooks#hydratemessages): the deprecated hook's reference entry
* [Database persistence](/docs/ai-chat/patterns/database-persistence): the hook-based pattern this replaces
