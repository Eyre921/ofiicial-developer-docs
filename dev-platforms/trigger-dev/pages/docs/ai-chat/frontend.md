...
`sendMessage` from `useChat` gives no feedback about whether the message actually reached the backend. The transport's `onEvent` callback closes that gap with typed lifecycle events (see the [event catalog](/docs/ai-chat/reference#transport-events)) so you can record real metrics:
