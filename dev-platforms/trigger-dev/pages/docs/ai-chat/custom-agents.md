...
path: docs/ai-chat/custom-agents
---
...
The frontend stops a turn with [`transport.stopGeneration(chatId)`](/docs/ai-chat/frontend#stop-generation), which writes a stop signal to the session's input stream. It aborts the current turn's generation but keeps the run alive, so the next message continues on the same session.
...
`chat.close({ reason })` works in a custom agent exactly as it does in [`chat.agent`](/docs/ai-chat/backend#ending-the-conversation): the session row is closed, further sends are refused with HTTP 409, and no continuation run is scheduled. Call it from anywhere in your loop.
