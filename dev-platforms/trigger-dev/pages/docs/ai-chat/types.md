...
`withClientData` also takes `reportErrorAt` and `onValidationError` alongside `schema`. See [chat.withClientData](/docs/ai-chat/reference#chatwithclientdata) for both, and [Validating client data](/docs/ai-chat/custom-agents#validating-client-data) for the custom-agent walkthrough.
