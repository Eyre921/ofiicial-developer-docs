...
To close from inside the agent instead, call [`chat.close()`](/docs/ai-chat/backend#ending-the-conversation). It writes a terminal record to the response stream so the browser learns the reason, then closes the row.
