...
path: docs/ai-chat/patterns/human-in-the-loop
---
...
A pause doesn't hold compute. After the model calls a no-execute tool, the turn finishes and the run stays warm for `idleTimeoutInSeconds` (default 30s), then **suspends** and frees its compute, the same way [`wait.for`](/docs/wait-for) does. The user's `addToolOutput` wakes it back up.

Because the run is suspended while it waits, the human's thinking time is not billed and does **not** count against [`maxDuration`](/docs/runs/max-duration). `maxDuration` measures active CPU time and excludes suspended waitpoint time, exactly like `wait.for`, so a user can take minutes, hours, or days to answer without the run hitting `maxDuration`. The only time that counts is each turn's actual compute plus the short warm window before each suspend.
