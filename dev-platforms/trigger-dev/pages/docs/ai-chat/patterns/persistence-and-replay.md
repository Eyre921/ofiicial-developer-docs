...
path: docs/ai-chat/patterns/persistence-and-replay
---
...
`state` holds what the runtime cannot rebuild from the messages, such as a [compaction](/docs/ai-chat/compaction) summary; `final` is false for a partial answer captured from a failed turn. Snapshots written by older SDK versions have `version: 1` and are read as if every message were final with no state.
...
A [transcript storage](/docs/ai-chat/transcript-storage) with `loadContext` gives you the same ownership of the model's context while the runtime keeps writing every change and its own state to your store. It is the recommended path; `hydrateMessages` is deprecated.
...
* [Transcript storage](/docs/ai-chat/transcript-storage) — the adapter the runtime persists through, and how to bring your own
* [`hydrateMessages`](/docs/ai-chat/lifecycle-hooks#hydratemessages) — the deprecated context hook
