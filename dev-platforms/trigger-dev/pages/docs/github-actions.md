...
<Warning>
  ...
  pinning](/docs/github-actions#cli-version-pinning) section for more details.
</Warning>
...
The `--external-id ${{ github.sha }}` above tags the deployment with the commit it was built from. That is the first half of [version skew protection](/docs/deployment/version-skew-protection): to complete it, give your running application the **same value** so it sends that id when it triggers.
...
<Note>
  ...
  lands](/docs/deployment/version-skew-protection#when-nothing-ever-lands).
</Note>
