...
path: docs/realtime/react-hooks/streams
---
...
`refreshAccessToken` is also available on [`useApiClient` and `TriggerAuthContext`](/docs/realtime/auth), so every hook under a provider shares one refresher.
