...
When you trigger a task from your backend code, create a named API key with **Trigger only** access in the environment you want to target and set it as `TRIGGER_SECRET_KEY`. If you're [using a preview branch](/docs/deployment/preview-branches), also set `TRIGGER_PREVIEW_BRANCH`. [More info on API keys](/docs/apikeys).
