...
path: docs/self-hosting/kubernetes
---
...
The official Helm chart installs the full Trigger.dev stack into a Kubernetes namespace. Read the self-hosting [overview](/docs/self-hosting/overview) first.
...
Most values map directly to the environment variables documented in the [webapp](/docs/self-hosting/env/webapp) and [supervisor](/docs/self-hosting/env/supervisor) environment variable overview. Environment variables use `UPPER_SNAKE_CASE`; Helm values use `camelCase`:
...
Object storage holds deploy artifacts and large payloads. See the [Docker object storage setup](/docs/self-hosting/docker#object-storage) for conceptual information. Disable the bundled MinIO and point at any S3-compatible endpoint.
...
Built deploy images are pushed to a registry. See the [Docker registry setup](/docs/self-hosting/docker#registry-setup) for conceptual information. Configure an external registry in your `values.yaml`:
...
Authentication options are identical to the [Docker-based installation](/docs/self-hosting/docker#authentication). Configure the provider in your `values.yaml`:
...
<Note>
  ...
  you still have v3 tasks, pin to exactly 4.5.0 or [migrate to v4](/docs/migrating-from-v3). 4.5.1 and
