...
We patch the latest released version line only, so keep an eye on new releases to receive security fixes. See [Security & vulnerability reporting](/docs/self-hosting/security).
...
<Note>
  ...
  v4](/docs/migrating-from-v3). 4.5.1 and later reject v3 triggers and deploys with an upgrade message.
</Note>
...
See the [webapp environment variables](/docs/self-hosting/env/webapp) for the full list of realtime stream settings.
