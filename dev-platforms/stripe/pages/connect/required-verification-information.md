...
path: connect/required-verification-information
---
...
- If there is ever a conflict between the user’s request and the validated setup, inform the user of the conflict and ask them to revise their setup choices using the [Interaction contract](https://docs.stripe.com/connect/required-verification-information.md#interaction-contract). Keep the validated setup aligned with what the user requested without silently dropping the conflict.
- Follow the [Interaction contract](https://docs.stripe.com/connect/required-verification-information.md#interaction-contract) for every user question.
- When the number of available options exceeds four, *always* print the full validated reference list before asking the multiple-choice question so the user can see the full option space.
  - When printing countries, always print the full country name followed by its code in parentheses, e.g. `Germany (DE)`.
  - Use specific countries in the quick-pick options so the user can move forward with immediate clarity.
  - Leave the descriptions for the country quick-pick options blank.
- For any field with a small valid option set, show the user every valid option.
...
- If the `businessStructure` map for the chosen `legalEntityType` is empty or contains exactly one key `nil`, skip `businessStructure`. Otherwise, ask for `businessStructure` and always allow a `none` option or leave unselected as a quick-pick option in the multiple choice question.
...
By the time you make the final requirements request, you must have a validated value for:
...
You also should have asked for the following optional fields, if needed:
...
5. If the filtered list is empty, tell the user that no capabilities are supported for the current setup and ask them to revise earlier setup choices using the [Interaction contract](https://docs.stripe.com/connect/required-verification-information.md#interaction-contract) before making the final requirements request.
...
8. Resolve and ask for `capabilities` using [Resolve capabilities](https://docs.stripe.com/connect/required-verification-information.md#resolve-capabilities).
9. Ask for `orrProgram` only if the validated setup exposes one or more public programs.
10. If the user’s requested setup doesn’t match the valid options, tell them exactly which parts are invalid or auto-adjusted, then ask the correcting follow-up using the [Interaction contract](https://docs.stripe.com/connect/required-verification-information.md#interaction-contract). Keep the mismatch visible, keep the setup grounded in the user’s request, and continue with a structured follow-up question.
...
If you are asked to compare two setups or are asked what is needed to update from X to Y, you must repeat the same validation flow independently for setup A and setup B with a second top-level key `account-setup-B[...]` before calling the diffable requirements request.
...
Ask the user which `platformCountry` value they want to use. Then, fetch the allowed options for that platform country. This request tells you what is valid next, and you must use it before choosing downstream fields. Assume the user chose `US`:
...
After that response returns, collect setup choices as described in the [Agent flow](https://docs.stripe.com/connect/required-verification-information.md#agent-flow) section.
...
Use this response to present the requirements to the user as explained in the [Construct the result](https://docs.stripe.com/connect/required-verification-information.md#construct-the-result) section.
...
- if you change `legalEntityType`, re-check `businessStructure`
- if you change `accountCountry`, `tosType`, or `apiVersion`, re-run [Resolve capabilities](https://docs.stripe.com/connect/required-verification-information.md#resolve-capabilities)
...
Check the supplemental endpoints to see if there is any additional capability-specific restrictions to present to the user.
...
3. For each table:
   - use the section name as the table heading, for example `Account`, `Company`, `Representative`, `Directors`, or `Owners`
   - Include the following columns:
     - Heading: blank
       - Content: Row display name, e.g. “Name”, “Date of birth”, “Address”
     - Heading: `Requirement`
       - Content: a bulleted list of displayed fields
       - Render one bullet per displayed field
       - Render each field in code formatting
       - If a field has alternatives, keep them in the same bullet and render them as a disjunction, for example ``field_a` or `field_b``
     - Heading: `Verification`
       - Content: a bulleted list built from `extras[].value`
       - Render each `extras[].value` entry as a bullet
     ...
     - Heading: `Enforcement action`
       ...
       - If both `if not provided` and `if not verified` text exist, render the `if not provided` sentence(s) first and then the `if not verified` sentence(s); prefix the first verification sentence with `Also,`
...
5. Assign each section to a row-definition family listed below in point 8. The row-definition family only controls how rows are matched and labeled inside that section’s table:
   - `account` -> `account`
   - `company` -> `entity`
   - `documents` -> `entity`
   - `individual` -> `person`
...
- if the user asks for a JSON summary of required items, return a JSON object in this exact shape:
  ```json
  {
    "requirements": {
      "currently_due": [
  ...
  ```
- populate `requirements.currently_due` with the requirement field names that are currently due and `requirements.eventually_due` with the requirement field names that are future or eventual requirements
- for `apiVersion=v2`, use `v2_field_name` values in both arrays; for `apiVersion=v1`, use the raw requirement field names
- the JSON diff view compares requirement names only; it doesn’t diff verification text, thresholds, or supplemental metadata

### How to respond back to users
...
- present currently due requirements separately from eventually due requirements and future requirements, and label them clearly
...
- if you receive `validation_errors`, ask the user to correct the setup inputs using the [Interaction contract](https://docs.stripe.com/connect/required-verification-information.md#interaction-contract) instead of guessing
- if you receive `build_errors`, retry the request and tell the user the helper endpoint failed unexpectedly if the error persists
