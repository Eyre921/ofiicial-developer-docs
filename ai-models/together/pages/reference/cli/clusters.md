...
| `--num-preemptible-gpus [integer]`     | Number of preemptible GPUs to request alongside standard capacity. Must be a multiple of 8. Requires a project (pass the global `--project` flag or set `TOGETHER_PROJECT_ID`). See [preemptible compute](/docs/preemptible-compute).                                                                      |
