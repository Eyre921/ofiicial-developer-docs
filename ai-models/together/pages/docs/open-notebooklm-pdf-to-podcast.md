---
title: "Build an open source NotebookLM"
source: https://docs.together.ai/docs/open-notebooklm-pdf-to-podcast
path: docs/open-notebooklm-pdf-to-podcast
---
...
1. How we can use JSON mode and structured generation with open models like Llama 3.3 70B Instruct Turbo to extract a script for the podcast given text from the PDF.
...
* The overall conversation consists of lines said by either the host or the guest. The `LineItem` class specifies the structure of these lines.
* The full script is a combination of multiple lines performed by the speakers, here we also include a `scratchpad` field to allow the LLM to ideate and brainstorm the overall flow of the script prior to actually generating the lines. The `Script` class specifies this.
...
Below we call Llama 3.3 70B Instruct Turbo with JSON mode to generate a script for our podcast. JSON mode makes it so that the LLM will only generate responses in the format specified by the `Script` class. We will also be able to read its scratchpad and see how it structured the overall conversation.
...
client_together = Together(api_key=os.environ["TOGETHER_API_KEY"])
...
[
    LineItem(
        speaker="Host (Jane)",
        text="Welcome to today’s podcast. I’m your host, Jane. Joining me is Junlin Wang, a researcher from Duke University and Together AI. Junlin, welcome to the show!",
    ),
    LineItem(
        speaker="Guest",
        text="Thanks for having me, Jane. I’m excited to be here.",
    ),
    LineItem(
        speaker="Host (Jane)",
        text="Junlin, your recent paper proposes a new approach to enhancing large language models (LLMs) by leveraging the collective strengths of multiple models. Can you tell us more about this?",
    ),
    LineItem(
        speaker="Guest",
        text="Our approach is called Mixture-of-Agents (MoA). We found that LLMs exhibit a phenomenon we call collaborativeness, where they generate better responses when presented with outputs from other models, even if those outputs are of lower quality.",
    ),
    LineItem(
        speaker="Host (Jane)",
        text="That’s fascinating. Can you walk us through how MoA works?",
    ),
    LineItem(
        speaker="Guest",
        text="MoA consists of multiple layers, each comprising multiple LLM agents. Each agent takes all the outputs from agents in the previous layer as auxiliary information in generating its response. This process is repeated for several cycles until a more robust and comprehensive response is obtained.",
    ),
    LineItem(
        speaker="Host (Jane)",
        text="I see. And what kind of results have you seen with MoA?",
    ),
    LineItem(
        speaker="Guest",
        text="We evaluated MoA on several benchmarks, including AlpacaEval 2.0, MT-Bench, and FLASK. Our results show substantial improvements in response quality, with MoA achieving state-of-the-art performance on these benchmarks.",
    ),
    LineItem(
        speaker="Host (Jane)",
        text="Wow, that’s impressive. What about the cost-effectiveness of MoA?",
    ),
    LineItem(
        speaker="Guest",
        text="We found that MoA can deliver performance comparable to GPT-4 Turbo while being 2x more cost-effective. This is because MoA can leverage the strengths of multiple models, reducing the need for expensive and computationally intensive training.",
    ),
    LineItem(
        speaker="Host (Jane)",
        text="That’s great to hear. Junlin, what do you think is the potential impact of MoA on the field of natural language processing?",
    ),
    LineItem(
        speaker="Guest",
        text="I believe MoA has the potential to significantly enhance the effectiveness of LLM-driven chat assistants, making AI more accessible to a wider range of people. Additionally, MoA can improve the interpretability of models, facilitating better alignment with human reasoning.",
    ),
    LineItem(
        speaker="Host (Jane)",
        text="That’s a great point. Junlin, thank you for sharing your insights with us today.",
    ),
    LineItem(
        speaker="Guest",
        text="Thanks for having me, Jane. It was a pleasure discussing MoA with you.",
    ),
]
```
...
client_cartesia = Cartesia(api_key=os.environ["CARTESIA_API_KEY"])
