...
An active [rollout](/docs/dedicated-endpoints/rollouts) also holds a replica floor for each deployment for its current step. If you lower `maxReplicas` below what the rollout's current step requires, the rollout pauses until you restore a compatible replica bound.
...
`--scaling-metric` accepts one of nine metrics, and `--scaling-target` sets the threshold the platform holds it near. The unit of the target depends on the metric:

| Metric (`--scaling-metric`) | What `--scaling-target` sets                                                                                                                          |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `inflight_requests`         | Concurrent in-flight requests per replica. The default metric; the target is `8` when no metric is specified. Works for LLM engines and TTS handlers. |
| `active_sessions`           | Concurrent open client WebSocket sessions per replica on a TTS deployment. Accepts `AVERAGE_VALUE` only.                                              |
...
* **Average value** (`inflight_requests`, `active_sessions`, `throughput_per_replica`): an average value across all replicas. `inflight_requests` with `--scaling-target 16` aims for about 16 concurrent requests per replica. `active_sessions` with `--scaling-target 4` aims for about four open WebSocket sessions per replica.
...
The nine metrics fall into three families. Which one fits depends on what you want to protect the deployment against.

* **Concurrency-driven (`inflight_requests`, `active_sessions`) is the safe default.** In-flight count is a *leading* indicator: it rises when demand outpaces service, but before latency visibly degrades. It needs no streaming and no percentile choice, and it maps directly onto how inference engines batch requests. A target of `8` says "hold each replica at about eight concurrent requests." Raise it for short-prompt chat workloads that batch well, and lower it for long-context traffic (like coding agents) that spends longer in prefill. For TTS WebSocket traffic, prefer `active_sessions` (sessions per replica). HTTP TTS traffic can keep using `inflight_requests`.
...
| ------------------------ | -------------------------------------------------------------------------- |
| `inflight_requests`      | Default choice. Robust, leading, and engine-agnostic (including HTTP TTS). |
| `active_sessions`        | TTS deployments with concurrent WebSocket sessions.                        |
