...
For Mixture-of-Experts (MoE) models like Kimi K2 (384 experts), training stability benefits from **Routing Replay**. Routing Replay captures the expert routing assignments used by the inference sampler and replays them during the policy training forward pass. This keeps training on the same expert paths that produced the rollout and reduces variance caused by routing changes.

To use Routing Replay with the Training API, request routing matrices while sampling, align them with the training input tokens, and pass them through `tinker.ModelInput.from_ints(..., routing_matrices=...)` when constructing the policy datum. You can inspect or customize the matrices before constructing the datum.
