...
| `glm-fast-latest`   | Latency-sensitive agentic use | Example mapping uses this for `opus` and `sonnet` in Claude Code. Version-tracking router on the high-speed Fast serving path (100+ tok/s), at a higher per-token price. 1M context. |
...
| `kimi-fast-latest`  | General use (lighter)         | Example mapping uses this for `main` and `fable` in Claude Code. Fast Kimi variant with vision support.                                                                              |
| `glm-5p1`           | General use (lighter)         | Pinned GLM 5.1; use with `--sonnet glm-5p1` if you prefer it over the example mapping.                                                                                               |
| `deepseek-v4-flash` | Background / fast tasks       | Example mapping uses this for `haiku` and `subagent` in Claude Code. Lowest latency.                                                                                                 |
...
Fire Pass keys (`fpk_...`) route all Claude Code slots to `kimi-fast-latest`. The model browser shows Fire Pass-supported routers: `glm-latest`, `glm-fast-latest`, `glm-5p2-fast`, `kimi-fast-latest`, and `kimi-k2p7-code-fast`.
...
Claude Code uses multi-slot aliases (`main`, `opus`, `sonnet`, `haiku`, `fable`, `subagent`). See [Claude Code](/ecosystem/fireconnect/claude-code#example-model-mapping) for the full mapping table.
