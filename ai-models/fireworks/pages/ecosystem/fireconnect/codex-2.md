...
* FireConnect **v0.9.5+** (see [Install](/ecosystem/fireconnect/overview#install))
...
`fireconnect codex` and `fireconnect chatgpt` share one config — both the [Codex CLI](https://developers.openai.com/codex) and the **ChatGPT desktop app** route through Fireworks with a single command.

<Note>
  **Quit the ChatGPT app** before `on` or `off` so its model list refreshes. Same quit rules as Cursor and VS Code.
</Note>
...
fireconnect chatgpt on            # Alias — same config as codex (includes ChatGPT desktop app)
