...
source: https://replicate.com/docs/topics/predictions/streaming.md
path: docs/topics/predictions/streaming
---
...
When you create a prediction via the API, the response includes a `urls` object which contains a number of URLs that you can use to interact with the prediction. If the requested model version supports streaming, then the `urls` object will include a `stream` URL:
...
You can use this `urls.stream` URL to receive streaming output using [server-sent events (SSE)](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events).
