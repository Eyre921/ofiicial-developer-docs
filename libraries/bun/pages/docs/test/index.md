...
Every shard must read the *same set* of timings files for the shards to add up to the whole suite, which is why a run reads the previous run's files (restored from the cache) and writes its own where sibling shards still in flight won't pick them up (`next/` above). Add `--no-isolate` to the `bun test` line if step 2 applies to you.
...
<ParamField type="number">
  Retry failed tests up to <code>NUMBER</code> times. Overridden by per-test&#x20;
</ParamField>
