...
<Update label="2026-09-01">
  ...
  Egress is metered on reads that return record data, such as query, fetch, list, and search. Writes, index statistics, and index management requests aren't metered. Egress applies to indexes that use [dedicated read nodes](/guides/index-data/dedicated-read-nodes) as well as on-demand indexes.
...
<Update label="2026-01-23">
  ...
  * **Post-filter client-side**: Retrieve a larger top K without filtering, then filter results in your application.
  ...
  For more information, see [Metadata filter limits](/reference/api/database-limits/operation-limits#metadata-filter-limits) and [Design for multi-tenancy](/guides/index-data/data-modeling#design-for-multi-tenancy).
</Update>
