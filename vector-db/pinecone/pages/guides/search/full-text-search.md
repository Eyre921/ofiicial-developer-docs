...
BM25 can also rank on several fields at once. Name multiple fields in one `text` clause, or pass multiple `text` clauses, and Pinecone combines them into a single ranking. A `query_string` clause can target several fields too. Every field contributes equally in `2026-07`, and there's no per-field weight parameter.
...
Everything in this section applies to indexes with document schemas. For dense, sparse, and integrated-inference indexes, see [Search overview](/guides/search/search-overview).
...
* **Document and schema limits**: See [Upsert limits](/reference/api/database-limits/operation-limits#upsert-limits) for fields per schema, field and token size, document size, request size, and metadata size.
* **Query limits**: See [Query limits](/reference/api/database-limits/operation-limits#query-limits) for the `top_k` and result-size caps. Full-text search is optimized for ranked retrieval rather than for aggregation or count-style queries.
* **Update limits**: See [Update limits](/reference/api/database-limits/operation-limits#update-limits) for how many documents one update request can patch.

### Supported operations

* **Partial updates**: Upsert replaces the whole document for a given `_id`. `POST /namespaces/{namespace}/documents/update` patches individual fields, by ID or by metadata filter. See [Update documents](/guides/manage-data/update-data#update-documents).
* **Bulk import** from object storage, via JSONL files. See [Prepare document-schema files (JSONL)](/guides/index-data/import-data#prepare-document-schema-files-jsonl).
