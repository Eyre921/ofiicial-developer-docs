---
title: "Manage serverless indexes"
...
<Note>
  On API version `2026-07`, the REST `describe_index` and `list_indexes` responses replace the top-level `dimension`, `metric`, `vector_type`, and `spec` with top-level `deployment`, `read_capacity`, and `schema` objects, so `dimension` and `metric` now appear on the schema's dense-vector field. The `curl` examples below show this shape. The SDKs return both the new fields and the older top-level ones for backward compatibility, so the SDK examples still show `dimension`, `metric`, and `spec`. See [Adopt the Documents API](/guides/index-data/adopt-the-documents-api) for details.
</Note>
...
<Note>
  Document-schema creation is currently available through the Python and JavaScript SDKs and REST. The Java and Go tabs create a classic vector index. See [Create an index](/guides/index-data/create-an-index).
</Note>

<CodeGroup>
  ```python Python theme={null}
  # pip install pinecone
  # Serverless index with a document schema
