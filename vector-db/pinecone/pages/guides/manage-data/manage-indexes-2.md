...
<Note>
  On API version `2026-07`, `describe_index` and `list_indexes` return a `schema` object in place of the top-level `dimension`, `metric`, and `vector_type`, so `dimension` now appears inside the schema's dense-vector field. The examples below show the earlier shape. See [Adopt the Documents API](/guides/index-data/adopt-the-documents-api) for details.
</Note>
