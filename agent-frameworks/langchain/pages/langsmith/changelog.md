...
<Tabs>
  <Tab title="LangSmith Cloud">
    <Update label="June 20-28, 2026">
      ## Observability and evaluations

      ### Tracing

      * Adding feedback or reviewer notes from the UI no longer upgrades short-lived traces to extended retention.

      ## Sandboxes

      * [Sandbox registries](/langsmith/sandbox-snapshots#private-registries) now support pulling private container images alongside existing sandbox and snapshot operations.
  ...
  <Tab title="LangSmith Fleet">
    <Update label="June 20-28, 2026">
      ...
      * Fleet chat now lets you create an [access profile](/langsmith/fleet/computer-use) directly from the dialog, so you can open the sandboxes create flow when a workspace has none configured.
      * Fleet now completes OAuth for [MCP servers](/langsmith/fleet/remote-mcp-servers) whose authorization server requires client-secret authentication at the token endpoint, so the consent flow finishes cleanly.
    </Update>
