...
source: https://docs.langchain.com/langsmith/javascript/managed-deep-agents-agent-owned-interrupts
path: langsmith/javascript/managed-deep-agents-agent-owned-interrupts
---
...
<Note>
Agent-owned interrupts require `managed-deepagents>=0.8.0` and a [Slack channel](/langsmith/javascript/managed-deep-agents-channels-slack) on a deployed agent. Forms render only in Slack. LangSmith Studio shows the raw interrupt value.
</Note>
...
Register the tool in the agent definition as you would any other [custom tool](/langsmith/javascript/managed-deep-agents-tools). The interrupt inherits the managed checkpointer, so no extra persistence setup is needed.
...
- [Human-in-the-loop](/langsmith/javascript/managed-deep-agents-tools#human-in-the-loop): pause before selected tool calls with platform-rendered approval cards.
- [Connect a Managed Deep Agent to Slack](/langsmith/javascript/managed-deep-agents-channels-slack): add the Slack channel that delivers form responses.
- [Manage connections](/langsmith/javascript/managed-deep-agents-connections): how `connections.get(...)` resolves agent-owned credentials.
- [Add custom tools](/langsmith/javascript/managed-deep-agents-tools): register the tool that posts the form.
