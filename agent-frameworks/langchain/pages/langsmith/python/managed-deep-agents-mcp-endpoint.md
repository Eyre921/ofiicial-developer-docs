...
path: langsmith/python/managed-deep-agents-mcp-endpoint
---
...
- **Tool name**: The agent `name` set in the [agent definition](/langsmith/python/managed-deep-agents-agent-definition#parameters).
...
To retain knowledge across calls, [opt in to durable memory](/langsmith/python/managed-deep-agents-memory). Durable memory is optional and shared by every caller of the deployment. Do not enable it when MCP clients should not influence one another.
