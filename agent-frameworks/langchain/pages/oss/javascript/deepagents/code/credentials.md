...
source: https://docs.langchain.com/oss/javascript/deepagents/code/credentials
path: oss/javascript/deepagents/code/credentials
---
...
For `.env` loading order and the `DEEPAGENTS_CODE_` prefix, see [Configuration](/oss/javascript/deepagents/code/configuration#environment-variables).
...
The `/auth` prompt also has an optional **base URL** field. Leave it blank to use the provider's default endpoint, or set a custom one to use with this key. The base URL is saved alongside the key. See [Endpoints, keys, and gateways](/oss/javascript/deepagents/code/config-file#endpoints-keys-and-gateways) for how endpoints resolve, including with gateways.
...
To keep keys in a file instead, define them in a [`.env` file](/oss/javascript/deepagents/code/configuration#environment-variables).

## Key resolution order

When a provider's key is set in more than one place, Deep Agents Code uses the first of these that is set:

1. **`DEEPAGENTS_CODE_`-prefixed env var** — for example `DEEPAGENTS_CODE_OPENAI_API_KEY` as an inline shell export. The [`DEEPAGENTS_CODE_` prefix](/oss/javascript/deepagents/code/configuration#deepagents_code_-prefix) is the explicit "use this key in Deep Agents Code" override.
...
Each provider's API key and its endpoint (`base_url`) resolve as a pair from the same source. See [Endpoints, keys, and gateways](/oss/javascript/deepagents/code/config-file#endpoints-keys-and-gateways).
  ...
  <Tab title="Set an environment variable">
    <Steps>
      ...
      <Step title="Add it to your environment">
        ...
        Shell exports take precedence over `.env` values (see [Loading order and precedence](/oss/javascript/deepagents/code/configuration#loading-order-and-precedence)). To scope a key to Deep Agents Code only without affecting other tools that read `TAVILY_API_KEY`, use the [`DEEPAGENTS_CODE_` prefix](/oss/javascript/deepagents/code/configuration#deepagents_code_-prefix): `DEEPAGENTS_CODE_TAVILY_API_KEY=tvly-...`.
      </Step>
